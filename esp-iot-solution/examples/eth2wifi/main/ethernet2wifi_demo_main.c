...
//use static ip. will not be used in this project. is just to avoid dhcp traffic.
//https://www.esp32.com/viewtopic.php?t=5380
    ...
    IP4_ADDR(&ip_info.ip, 192, 168, 4, 3);
    IP4_ADDR(&ip_info.gw, 192, 168, 4, 199);
    IP4_ADDR(&ip_info.netmask, 255, 255, 255, 0);
    ret = tcpip_adapter_set_ip_info(TCPIP_ADAPTER_IF_STA, &ip_info);
    printf("fixex ip ret = %x\n",ret);
...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT) {
        ...
        case WIFI_EVENT_STA_CONNECTED:
            ...
            esp_wifi_internal_reg_rxcb_compat(ESP_IF_WIFI_STA, (wifi_rxcb_t)tcpip_adapter_sta_input_eth_output); // das hat keine funktion mehr - workaround siehe esp_netif_lwip.c
            break;
        case WIFI_EVENT_STA_DISCONNECTED:
            printf("WIFI_EVENT_STA_DISCONNECTED\n");
            wifi_is_connected = false;
            esp_wifi_internal_reg_rxcb_compat(ESP_IF_WIFI_STA, NULL);
            esp_wifi_connect();
            break;
